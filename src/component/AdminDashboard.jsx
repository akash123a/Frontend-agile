    ...
    <div className="bg-gray-100 min-h-screen flex flex-col">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center" onClick={() => setIsModalOpen(false)}>
          <div className="bg-white rounded-lg shadow-lg w-96 p-6 relative" onClick={(e) => e.stopPropagation()}>
            <button onClick={() => setIsModalOpen(false)} className="absolute top-2 right-2 text-gray-600 hover:text-red-500">✖</button>
            
            <div className="flex border-b mb-4">
              {["employeeLogin", "organisationSignup"].map((tab) => (
                <button
                  key={tab}
                ...
                >
                  {tab === "employeeLogin" ? "Employee Login" : "Organization Signup"}
                </button>
              ))}
            </div>
            ...
            {activeTab === "employeeLogin" ? (
              <form>
                <input type="email" placeholder="Email" className="w-full p-2 border rounded-lg mb-4" />
                <input type="password" placeholder="Password" className="w-full p-2 border rounded-lg mb-6" />
                <button className="w-full p-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600">Login</button>
              </form>
            ) : (
              <form>
                <input type="text" placeholder="Organization Name" className="w-full p-2 border rounded-lg mb-4" />
                <input type="email" placeholder="Admin Email" className="w-full p-2 border rounded-lg mb-4" />
                <input type="password" placeholder="Password" className="w-full p-2 border rounded-lg mb-6" />
                <button className="w-full p-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600">Signup</button>
              </form>
            )}
          </div>
        </div>
